...
LEFT JOIN bundle_items bi ON bi.bundle_id = eo.bundle_id
  ...
  if (!rows[0]) return res.status(404).json({ error: 'Run not found' });
  const run = rows[0];
  if (run.user_id !== userId && req.session.role !== 'admin') {
  ...
  if (diff > 0 && pricePerUnit > 0) {
    ...
    if (!w[0] || Number(w[0].balance) < extraCost) {
    ...
    process.stderr.write(`[reschedule] charged ${extraCost} from user ${userId}\n`);
  }

  await query(
    `UPDATE organic_run_schedule
        SET quantity_to_send=$1, base_quantity=$1, scheduled_at=$2,
            status='pending', error_message=NULL, provider_order_id=NULL,
            started_at=NULL, completed_at=NULL, retry_count=0
      WHERE id=$3`,
    [newQty, scheduledAt, req.params.runId]
  );
